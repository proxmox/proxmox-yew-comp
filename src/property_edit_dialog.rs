...
#[doc(hidden)]
pub struct PwtPropertyEditDialog {
    loading: bool,
...
impl Component for PwtPropertyEditDialog {
    type Message = Msg;
    type Properties = PropertyEditDialog;
...
impl From<PropertyEditDialog> for VNode {
    fn from(props: PropertyEditDialog) -> Self {
        let key = props.key.clone();
        let comp = VComp::new::<PwtPropertyEditDialog>(Rc::new(props), key);
        VNode::from(comp)
    }
}
